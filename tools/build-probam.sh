...
usage()
{
	...
	echo -e "  build-probam.sh [options] device"
	echo -e ""
...
echo -e ${cya}"Building ${bldgrn}ProBam ${bldylw}v$VERSION"${txtrst}

# PROBAM device dependencies
echo -e ""
echo -e ${bldblu}"Looking for PROBAM product dependencies${txtrst}"${cya}
vendor/probam/tools/getdependencies.py "$device"
echo -e "${txtrst}"
...
rm -f out/target/product/$device/probam_*-ota*.zip
